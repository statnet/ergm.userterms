...
\pkg{statnet}}
\Plaintitle{ergm.userterms: A Template for Building 
Plug-in Packages for statnet}
...
for \code{netstat1} through \code{netstat3}, is given in \citet{ergmtermsjss}.
...
Undirected networks are effectively stored as directed networks in \pkg{ergm} but with
...
\code{TAIL(i)}, \code{HEAD(i)}.  The logic is that the proposal could possibly involve
...
values of that attribute for all the nodes. Within the \pkg{statnet} suite,
...
There terms will be computed at the \proglang{C} level and operate at native speeds. In
...
We would like to thank the \pkg{statnet} team, including Martina Morris, Carter
