...
$q(\vec x, \vec y^k)$ of being selected, where $q$ is some probability function known 
to the user.  (The $q$ function can and, in general, does place probability zero on
some values of $\vec x$, depending on the value of $\vec y^k$.)  
The Hastings ratio (b) is equal to
\begin{equation}\label{hastings}
\frac{P_{\vec\theta_0} (\vec Y=\vec y^k)}{P_{\vec\theta_0} (\vec Y=\vec x)}
\frac{q (\vec y^k, \vec x)}{q (\vec x, \vec y^k)},
\end{equation}
...
\[
\vec y^{k+1} = \cases{\vec x & if $u \le \mbox{Hastings ratio}$; \cr
...
to those that differ from $\vec y^k$ by exactly one edge indicator;  in other words, $\vec x$
involves a single edge toggle of $\vec y^k$.  Suppose that $\vec x$ is identical to $\vec y^k$
in all except the $(i,j)$ entry.  Notationally, we would write $x_{ij}=1-y_{ij}^k$ but
$\vec x_{ij}^c=\vec (y^k_{ij})^c$, where $\vec x_{ij}^c$ denotes the entire network $\vec x$ 
...
\frac{q (\vec y^k, \vec x)}{q (\vec x, \vec y^k)} \exp\{\pm \vec\theta_0^\top \delta (\vec y^k)_{ij} \},
\end{equation}
where 
