...
\Abstract {
Exponential-family random graph models (ERGMs) represent a powerful and flexible
...
\frac{P_{\vec\theta_0} (\vec Y=\vec z)}{P_{\vec\theta_0} (\vec Y=\vec y^k)}
...
and writing and compiling \proglang{C} code. These steps require additional
...
\code{varnames=NULL}.
\item[Line 4:] In this example, the argument \code{attrname} is of type
\code{character} and the argument \code{pow} is of type \code{numeric}.
\item[Lines 5 and 6:] The \code{attrname} argument is required; i.e., an error results
...
\item [Line 12:]  The \code{coef.names} argument 
...
\pkg{ergm.userterms} package are found in the \code{changestats.users.c}
file.
...
executed by the \code{TOGGLE_IF_MORE_TO_COME(i)} 
...
\code{UNDO_PREVIOUS_TOGGLES(i)} function.
...
\code{changestats.c} file in the \pkg{ergm} package.
...
Let us now turn to the \proglang{C} code. Here again, we can choose to add our code
to the existing file \code{changestats.users.c} in the \code{src} directory of the
...
\end{CodeInput}
\end{CodeChunk}
The nodal attribute-specific version can then follow an \code{else} statement.
...
Thus, following 
\code{if(attrflag==0)\{}, we should have
\begin{CodeChunk}
\begin{CodeInput}
...
{Simple Illustration of the \code{mindegree} Term}

Now that we have created the \code{mindegree} term, here is a simple check that
it works in a synthetic ten-node example.  We begin by simulating the network;
by default, the \code{network} function will randomly create edges in such a way
that the expected number of edges equals the number of nodes.
(It may be necessary to install the \pkg{ergm.userterms} package,
along with the \pkg{ergm} and \pkg{network} packages, before
typing this code.)
\begin{CodeChunk}
\begin{CodeInput}
...
\end{CodeInput}
\end{CodeChunk}
Next, add some simple nodal covariates, as a variable called \code{labels}.
\begin{CodeChunk}
\begin{CodeInput}
...
\end{CodeOutput}
\end{CodeChunk}
The new \code{mindegree} term may of course be used not just for
...
Additional examples abound within the \code{src/changestats.c} file in the source
code of the \pkg{ergm} package; 
...
of the \pkg{ergm.userterms} package, although their use will always remain
optional.
